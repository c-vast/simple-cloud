...
import com.vast.common.web.exception.GlobalException;
...
public class GlobalExceptionHandler {
    ...
    public Result<?> globalExceptionHandler(GlobalException e, HttpServletRequest request) {
        log.error("发生异常：{} -> {}", e.getCode(), e.getMessage(),e);
        return ResultWapper.failure(e.getCode(), e.getMessage());
    }

    @ExceptionHandler(BusinessException.class)
    public Result<?> businessExceptionExceptionHandler(BusinessException e, HttpServletRequest request) {
    ...
    public Result<?> handlerThrowable(Throwable e, HttpServletRequest request) {
        ...
        return ResultWapper.failure(ResultCode.SYSTEM_ERROR);
    }
    ...
    public Result<?> handlerValidationException(ValidationException e, HttpServletRequest request) {
        ...
        return ResultWapper.failure(ResultCode.PARAM_VALIDATION_ERROR,e.getMessage());
    }

    @ExceptionHandler(BindException.class)
    public Result<?> handleBindException(BindException e, HttpServletRequest request) {
        log.error("发生参数绑定异常：{}",e.getMessage(), e);
        return ResultWapper.failure(ResultCode.PARAM_TYPE_BIND_ERROR,e.getAllErrors().get(0).getDefaultMessage());
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    public Result<?> handleMethodArgumentNotValidException(MethodArgumentNotValidException e, HttpServletRequest request) {
        log.error("请求接口MethodArgumentNotValidException异常：{}",e.getMessage(),e);
        return ResultWapper.failure(ResultCode.PARAM_IS_INVALID,e.getBindingResult().getAllErrors().get(0).getDefaultMessage());
    }

    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    public Result<?> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException e, HttpServletRequest request) {
        log.error("请求接口MethodArgumentTypeMismatchException异常：{}",e.getMessage(),e);
        return ResultWapper.failure(ResultCode.PARAM_TYPE_BIND_ERROR);
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public Result<?> handleHttpMessageNotReadableException(HttpMessageNotReadableException e, HttpServletRequest request) {
        log.error("请求接口HttpMessageNotReadableException异常：{}",e.getMessage(),e);
        return ResultWapper.failure(ResultCode.FAILURE);
    }
    ...
    public Result<?> handleMissingServletRequestParameterException(MissingServletRequestParameterException e, HttpServletRequest request) {
        ...
        return ResultWapper.failure(ResultCode.PARAM_NOT_COMPLETE);
    }

    @ExceptionHandler(IllegalArgumentException.class)
    public Result<?> handleIllegalArgumentException(IllegalArgumentException e,HttpServletRequest request){
        log.error("请求接口IllegalArgumentException异常：{}",e.getMessage(),e);
        return ResultWapper.failure(ResultCode.PARAM_ERROR,e.getMessage());
    }
}
